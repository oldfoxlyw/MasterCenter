...
class Complete_mission extends CI_Controller
{
	private $pageName = 'master/complete_mission';
	private $user = null;
	...
	public function send()
	{
		...
		if(!empty($nickname) && !empty($ip))
		{
			...
			$result = $this->connector->post($ip . '/ser_complete_all_main_missions', $parameter, FALSE);
			
			$this->load->model('mlog');
			$this->mlog->writeLog($this->user, 'complete_mission/send');
